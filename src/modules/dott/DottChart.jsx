  ...
  return (
    <div className="chart-row" title={row.title}>
      ...
      <div className="chart-track">
        ...
        <span
          className={`chart-val ${zero ? '' : pos ? 'chart-val-pos' : 'chart-val-neg'}`}
          style={
            zero
...
export default function DottChart({ groups, measureLabel }) {
        ...
        (g) =>
          g.rows.length > 0 && (
            <div key={g.key} className="chart-group">
              ...
              {g.rows.map((r) => (
                <Bar key={r.key} row={r} scaleMax={scaleMax} />
              ))}
            </div>
          ),
      )}
